...
class Fighter {
  ...
  update(dt, input) {
    ...
    if (inAttack) {
      ...
      if (this.state === ST.SPECIAL && !this.spawnProjectile && this.stateTime > 140) {
...
function startRound() {
  game.p1 = new Fighter(1, 300, false, { body: '#36ab53', head: '#dfb17f' });
  game.p2 = new Fighter(2, 980, true, { body: '#3e72e0', head: '#dfb17f' });
  game.projectiles = [];
...
function readInput() {
  return {
    ...
    p2: {
      left: keys.has('ArrowLeft'), right: keys.has('ArrowRight'), up: keys.has('ArrowUp'),
      light: keys.has('Numpad1'), heavy: keys.has('Numpad3'), special: keys.has('Numpad2'), block: keys.has('Numpad0'),
    },
  };
}
...
function update(dt, now) {
  ...
  if (timeout || ko) {
    ...
    else { game.message = '¡Blue gana!'; game.wins2 += 1; }
...
function drawHud() {
  ...
  ctx.textAlign = 'left';
  ctx.fillText(`Rondas: ${game.wins1}`, 30, 78);
  ctx.textAlign = 'right';
  ctx.fillText(`Rondas: ${game.wins2}`, CFG.w - 30, 78);
}
...
function render() {
  drawBackground();
  if (game.state === 'menu') {
    drawOverlay('KIKEFIGHT WEB', 'Pulsa Enter para empezar');
    return;
  }
  ...
  if (game.state === 'game_over') drawOverlay(game.wins1 > game.wins2 ? '¡Kike campeón!' : '¡Blue campeón!');
}
